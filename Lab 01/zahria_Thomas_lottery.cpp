...
void SortAwards() {
	...
	//int minIndex = 30000000;
	cout << "The awards in decreasing order:" << endl;
	for (int i = 0; i < records-1; i++)
	{
		int max = gSortedAward[i];
		int maxIndex = i;

		for (int x = i + 1; x < records; x++)
		{
			if (max < gSortedAward[x])
			{
				max = gSortedAward[x];
				maxIndex = x;
			}
		...
		if (i != maxIndex)
		{
			gSortedAward[maxIndex] = gSortedAward[i];
			gSortedAward[i] = max;
		}
...
void DoAwardStatistics() {
	...
	//cout << sum << endl;
	average = double(sum) / double(records);
	// END OF TODO 3.=============================================
	...
	for (int i = 0; i < records; i++)
	{
		distance = pow(average - double(gPlayerAward[i]), 2);
		std += abs(distance);
	}
	std = sqrt(std / double(records));
	// END OF TODO 4.=============================================
